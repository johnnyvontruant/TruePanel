...
def test_each_mission_passes_operator_visible_acceptance(tmp_path):
    for name in mission_names():
        report = run_mission_report(name, runtime_dir=tmp_path / name)
...
def test_each_mission_passes_temporal_semantics(tmp_path):
    for name in mission_names():
        report = run_mission_report(name, runtime_dir=tmp_path / f"temporal-{name}")
...
def test_fan_stall_is_visible_and_recovers_in_health_model(tmp_path):
    report = run_mission_report("fan-stall-recovery", runtime_dir=tmp_path / "fan")
    checks = {item["check_id"]: item["passed"] for item in report["mission_control_acceptance"]["checks"]}
...
def test_fan_stall_debounces_once_and_recovers_once(tmp_path):
    report = run_mission_report("fan-stall-recovery", runtime_dir=tmp_path / "fan-temporal")
    checks = {item["check_id"]: item["passed"] for item in report["temporal_semantics"]["checks"]}
...
def test_storage_fault_transition_is_not_repeated(tmp_path):
    report = run_mission_report("drive-failure", runtime_dir=tmp_path / "drive-failure-temporal")
    checks = {item["check_id"]: item["passed"] for item in report["temporal_semantics"]["checks"]}
...
def test_storage_removal_transition_is_not_repeated(tmp_path):
    report = run_mission_report("drive-removal", runtime_dir=tmp_path / "drive-removal-temporal")
    checks = {item["check_id"]: item["passed"] for item in report["temporal_semantics"]["checks"]}
    assert report["storage_event_count"] == 1
...
def test_drive_failure_round_trip_is_visible_and_emits_one_recovery(tmp_path):
    report = run_mission_report("drive-failure-recovery", runtime_dir=tmp_path / "drive-failure-recovery")
    acceptance = {item["check_id"]: item["passed"] for item in report["mission_control_acceptance"]["checks"]}
    temporal = {item["check_id"]: item["passed"] for item in report["temporal_semantics"]["checks"]}
    assert report["storage_event_count"] == 2
    assert report["final"]["pool_health"]["HDDs"] == "ONLINE"
    assert acceptance["mission_control.storage_degraded_visible"] is True
    assert acceptance["mission_control.storage_recovery_visible"] is True
    assert acceptance["mission_control.storage_terminal_nominal"] is True
    assert temporal["temporal.storage_fault_degraded_persists"] is True
    assert temporal["temporal.storage_recovery_event_once"] is True
    assert temporal["temporal.storage_pool_recovers_at_event"] is True


def test_drive_reinsert_round_trip_is_visible_and_emits_one_detection(tmp_path):
    report = run_mission_report("drive-removal-reinsert", runtime_dir=tmp_path / "drive-removal-reinsert")
    acceptance = {item["check_id"]: item["passed"] for item in report["mission_control_acceptance"]["checks"]}
    temporal = {item["check_id"]: item["passed"] for item in report["temporal_semantics"]["checks"]}
    assert report["storage_event_count"] == 2
    assert report["final"]["pool_health"]["HDDs"] == "ONLINE"
    ...
    assert acceptance["mission_control.storage_recovery_visible"] is True
    assert acceptance["mission_control.storage_terminal_nominal"] is True
    assert temporal["temporal.storage_removal_degraded_persists"] is True
    assert temporal["temporal.storage_reinsert_event_once"] is True
    assert temporal["temporal.storage_reinsert_pool_recovers_at_event"] is True


def test_network_and_front_panel_outages_persist_until_recovery(tmp_path):
    network = run_mission_report("network-flap", runtime_dir=tmp_path / "network-temporal")
    lcd = run_mission_report("lcd-loss-recovery", runtime_dir=tmp_path / "lcd-temporal")
    network_checks = {item["check_id"]: item["passed"] for item in network["temporal_semantics"]["checks"]}
    lcd_checks = {item["check_id"]: item["passed"] for item in lcd["temporal_semantics"]["checks"]}
...
def test_stale_telemetry_persists_until_recovery_event(tmp_path):
    report = run_mission_report("stale-telemetry-recovery", runtime_dir=tmp_path / "stale-temporal")
    checks = {item["check_id"]: item["passed"] for item in report["temporal_semantics"]["checks"]}
...
def test_thermal_recommendation_is_visible_through_status_bridge(tmp_path):
    report = run_mission_report("thermal-ramp", runtime_dir=tmp_path / "thermal")
    checks = {item["check_id"]: item["passed"] for item in report["mission_control_acceptance"]["checks"]}
    ...
    assert checks["mission_control.thermal_recommendation_matches_policy"] is True


def test_stale_telemetry_is_visible_and_recovers_in_mission_control(tmp_path):
    report = run_mission_report("stale-telemetry-recovery", runtime_dir=tmp_path / "stale")
    checks = {item["check_id"]: item["passed"] for item in report["mission_control_acceptance"]["checks"]}
...
def test_flight_deck_requires_all_mission_control_acceptance(tmp_path):
    ...
    assert all(item["mission_control_acceptance_passed"] for item in report["missions"])
    assert all(item["temporal_semantics_passed"] for item in report["missions"])
