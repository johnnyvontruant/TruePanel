...
def advance_to_end(provider):
    scenario_end = max(
        (event.at for event in provider._events),
        default=0,
    )
...
def test_missions_are_deterministic(name):
    ...
    assert [event.type for event in first.applied_events] == [
        event.type for event in mission_scenario(name).events
    ]
...
def test_drive_failure_degrades_pool():
    provider = provider_for("drive-failure")
    state = provider.advance(35)
...
def test_drive_removal_marks_bay_absent_and_degrades_pool():
    provider = provider_for("drive-removal")
    state = provider.advance(35)
