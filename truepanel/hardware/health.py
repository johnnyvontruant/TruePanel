...
def classify_storage_telemetry(
    ...
    *,
    ...
    """Classify one storage telemetry sample using canonical health policy."""
    ...
    if (
        ...
        and telemetry.temperature_c >= critical_temperature_c
    ):
        return (
    ...
    if (
        ...
        and telemetry.percentage_used >= nvme_critical_percentage_used
    ):
        return (
            HealthState.CRITICAL,
    ...
    if (
        telemetry.temperature_c is not None
        and telemetry.temperature_c >= warning_temperature_c
    ):
        return (
            HealthState.WARNING,
            f"temperature {telemetry.temperature_c}°C",
    ...
    if (
        telemetry.percentage_used is not None
        and telemetry.percentage_used >= nvme_warning_percentage_used
    ):
        return (
            HealthState.WARNING,
            f"NVMe endurance used: {telemetry.percentage_used}%",
...
class StorageHealthService:
    ...
    def classify(
        ...
        return classify_storage_telemetry(
            telemetry,
            warning_temperature_c=self.warning_temperature_c,
            critical_temperature_c=self.critical_temperature_c,
            nvme_warning_percentage_used=(
                self.nvme_warning_percentage_used
            ),
            nvme_critical_percentage_used=(
