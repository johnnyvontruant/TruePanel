...
function consequenceView(context){
    if(!context||typeof context!=="object") return "";
    if(context.state!=="PROVED") return `<section class="ag-consequence hold" aria-label="Impact topology coverage gap"><div><span>IMPACT TOPOLOGY · HOLD</span><h4>Dependency reach unavailable</h4></div><p>${esc(context.coverage_gap||"No exact proved dependency context is available for this incident.")}</p><small>Diagnostic confidence unchanged · no dependency inferred</small></section>`;
    const counts=context.counts||{};
    const paths=Array.isArray(context.proved_paths)?context.proved_paths:[];
    const labels=paths.filter(item=>["application","cargo","backup_evidence"].includes(item?.kind)).slice(0,4).map(item=>item?.label||item?.node_id).filter(Boolean);
    const pathChanged=context.source_device&&context.sentinel_device&&context.source_device!==context.sentinel_device;
    const identityState=context.stable_identity?"LIFELINE ID VERIFIED":"LIFELINE ID MISSING";
    const address=pathChanged?`${context.source_device} → ${context.sentinel_device}`:(context.sentinel_device||context.source_device||"Exact device");
    return `<section class="ag-consequence" aria-label="Proved dependency reach"><div><span>PROVED DEPENDENCY REACH · ${identityState}</span><h4>${esc(address)}</h4></div><div class="ag-consequence-counts"><strong>${esc(counts.proved_objects??0)} objects</strong><span>${esc(counts.applications??0)} app · ${esc(counts.cargo??0)} cargo · ${esc(counts.verified_backup_evidence??0)} verified backup</span></div><p>${labels.length?esc(labels.join(" · ")):"No application or cargo object is in the proved path."}</p><small>${pathChanged?"Linux address changed; stable hardware identity matched. · ":""}${esc(context.language_guard||"Reachability does not prove an outage.")}</small></section>`;
}
...
function render(view,payload){
    ...
    const coverageReview=reliability?.coverage_review||{};
    const envelopeDraft=reliability?.coverage_envelope_draft||{};
    const envelopeReview=reliability?.coverage_envelope_review||{};
    ...
    const reviewStatus=coverageReview.status||"HOLD";
    const envelopeStatus=envelopeDraft.status||"HOLD";
    const envelopeReviewStatus=envelopeReview.status||"HOLD";
    const candidatePanel=Object.keys(candidate).length?`<details class="ag-coverage ag-coverage-candidate"><summary>Recovery Coverage Next · Identity <span>${esc(title(candidateStatus))}</span></summary><div class="ag-evidence-metric"><strong>${esc(identityCodes.length)} storage recovery classes identity-bound</strong><br>Serial-model, ZFS-member, and WWN continuity are rehearsed across device-path change. This candidate is not accepted and cannot alter the current airworthiness envelope.<br>Appraisal: ${esc(title(appraisalStatus))} · Independent review: ${candidate.review_required===true?"REQUIRED":"HOLD"} · Automatic acceptance: ${candidate.automatic_acceptance===false?"DISABLED":"HOLD"} · Control authority: NO</div><div class="ag-evidence-metric"><strong>Content bindings</strong><br>Accepted predecessor, candidate, identity evidence, appraisal policy, and evaluator implementation must all match. Independent review complete: ${appraisal.independent_review_complete===true?"YES":"NO"} · Candidate accepted: ${appraisal.candidate_accepted===true?"YES":"NO"}</div><div class="ag-evidence-metric"><strong>Review handoff · ${esc(title(reviewStatus))}</strong><br>The exact packet requires two operator-owned Ed25519 signatures before AEGIS may call it eligible for a successor-envelope draft. Production keys present: ${coverageReview.production_keys_present===true?"YES":"NO"} · Successor envelope created: ${coverageReview.successor_envelope_created===true?"YES":"NO"} · Candidate accepted: ${coverageReview.candidate_accepted===true?"YES":"NO"}</div><div class="ag-evidence-metric"><strong>Envelope draft · ${esc(title(envelopeStatus))}</strong><br>${esc(envelopeDraft.reason||"Independent review evidence is required before drafting.")} · Draft created: ${envelopeDraft.successor_envelope_created===true?"YES":"NO"} · Envelope accepted: ${envelopeDraft.successor_envelope_accepted===true?"YES":"NO"} · Installed: ${envelopeDraft.candidate_installed===true?"YES":"NO"}</div><div class="ag-evidence-metric"><strong>Envelope review · ${esc(title(envelopeReviewStatus))}</strong><br>${esc(envelopeReview.reason||"Two independent envelope signatures are required.")} · Valid reviewers: ${esc(envelopeReview.valid_reviewer_count??0)}/2 · Manual acceptance required: ${envelopeReview.manual_acceptance_required===true?"YES":"HOLD"} · Envelope accepted: ${envelopeReview.envelope_accepted===true?"YES":"NO"} · Installed: ${envelopeReview.envelope_installed===true?"YES":"NO"}</div></details>`:"";
