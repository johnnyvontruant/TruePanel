...
function render(view,payload){
    ...
    const developmentPanel=Object.keys(developmentReview).length?`<section class="ag-dev-review" aria-label="AEGIS development review authority"><div><span>DEVELOPMENT REVIEW</span><strong>${esc(developmentReview.label||"Single operator · development only")}</strong><small>${esc(title(developmentReview.status||"hold"))} · JT is the sole human approver; Vega review is evidence only.</small><small>Public key roster · ${esc(title(developmentReview.public_key_roster||"not provisioned"))} · Operator signature · ${esc(title(developmentReview.operator_signature||"awaiting"))}</small><small>Signing session · clean pinned checkout + commit tree + operator-confirmed UTC required</small><small>Offline tool · export public kit · require two audit implementations to agree · sign outside TruePanel · verify returned signature</small><small>Independent audit witness · separately pinned standard-library verifier · matching session, card, and manifest digests required</small><small>Private key never accepted · review card is derived, canonical JSON is signed</small></div><div class="ag-dev-authority"><span>Production authority · NO</span><span>Deployment · NO</span><span>Hardware · NO</span><span>Storage writes · NO</span></div></section>`:"";
