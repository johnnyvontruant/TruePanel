...
function activity(payload){
    ...
    if(!block||typeof block!=="object"||block.unavailable===true){
        return {label:"ACTIVITY UNAVAILABLE",detail:"OBSERVATORY evidence unavailable"};
    }
    const observations=Array.isArray(block.observations)?block.observations:[];
    if(!observations.length){
        return {label:"NO OBSERVED ACTIVITY",detail:"No normalized workload evidence"};
    }
...
function install(){
    ...
    style.textContent=`:root{--gc-focus:var(--warn);--gc-panel:var(--panel-solid);--gc-border:color-mix(in srgb,var(--edge) 28%,transparent)}#glassCockpitSituation{grid-column:1/-1;padding:1rem 1.15rem;border-color:var(--gc-border);background:linear-gradient(120deg,color-mix(in srgb,var(--panel-solid) 72%,transparent),color-mix(in srgb,var(--panel-solid) 72%,transparent));backdrop-filter:blur(18px) saturate(180%);-webkit-backdrop-filter:blur(18px) saturate(180%)}.gc-now{display:grid;grid-template-columns:.55fr 1.2fr 1.4fr .75fr;gap:.7rem}.gc-now>div,.gc-domains section,.gc-activity{display:grid;align-content:start;gap:.3rem;min-width:0;padding:.7rem;border:1px solid var(--gc-border);border-radius:8px}.gc-now small,.gc-domains small,.gc-activity small{color:var(--muted);font-size:.62rem;font-weight:850;letter-spacing:.1em}.gc-now strong,.gc-domains strong,.gc-activity strong{overflow-wrap:anywhere}.gc-state{color:var(--warn)}.gc-activity{margin-top:.7rem;grid-template-columns:minmax(0,1fr);background:color-mix(in srgb,var(--accent-soft) 36%,transparent)}.gc-activity span{color:var(--muted);font-size:.68rem}.gc-domains{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:.7rem;margin-top:.7rem}.gc-domains span,.gc-trend{color:var(--muted);font-size:.68rem}.gc-domains svg{width:100%;height:30px}.gc-domains polyline{fill:none;stroke:var(--accent);stroke-width:2;vector-effect:non-scaling-stroke}#glassCockpitSituation details{margin-top:.65rem}#glassCockpitSituation summary{min-height:44px;display:flex;align-items:center;cursor:pointer;color:var(--muted)}:focus-visible{outline:3px solid var(--gc-focus)!important;outline-offset:3px}@media(prefers-reduced-motion:reduce){*,*::before,*::after{scroll-behavior:auto!important;animation:none!important;transition:none!important}}@media(max-width:760px){.gc-now,.gc-domains{grid-template-columns:1fr}.gc-now>div,.gc-domains section,.gc-activity{padding:.75rem}#glassCockpitSituation{padding:.85rem}}`;
