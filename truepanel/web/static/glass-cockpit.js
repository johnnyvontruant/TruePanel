...
function renderSentinel(view,payload){
    ...
    view.innerHTML=`<header class="sentinel-header"><div><small>SENTINEL</small><h3>Flight Director</h3></div><div class="sentinel-header-state"><span>${explanations.length} active explanation${explanations.length===1?"":"s"}</span><span class="sentinel-assessment">Assessment ${esc(assessmentState)}</span></div></header>${explanations.map(explanationMarkup).join("")}`;
}
