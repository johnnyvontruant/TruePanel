...
function installRefractionExperiment(){
    ...
    const filterValue='url("#gcRefraction") contrast(108%)';
    const supported=window.CSS?.supports?.("filter",filterValue)===true;
    if(!supported){
        document.body.dataset.gcRefraction="fallback";
        return;
    }
    ...
    pane.classList.add("gc-refraction-substrate");
    document.body.dataset.gcRefraction="substrate";
}
...
/* gc-health-nav-end */
/* gc-refraction-start: opt-in static SVG displacement of copied substrate; v1 glass is the fallback */
.gc-refraction-substrate .health-subsystem{position:relative;isolation:isolate;overflow:hidden}
.gc-refraction-substrate .health-subsystem::before{content:"";position:absolute;inset:-14px;border-radius:inherit;pointer-events:none;z-index:0;background:var(--gc-ambient-grid),linear-gradient(180deg,var(--gc-ambient-a),var(--gc-ambient-b));background-size:144px 83px,auto;background-attachment:fixed,fixed;filter:url("#gcRefraction") contrast(108%) saturate(104%);opacity:.52}
.gc-refraction-substrate .health-subsystem>*{position:relative;z-index:1}
@media(forced-colors:active){.gc-refraction-substrate .health-subsystem::before{display:none}}
/* gc-refraction-end */
