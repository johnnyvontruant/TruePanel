        ...
        hangar_flight_check = run(
            [
                ...
                (
                    ...
                    "development_review = next(item for item in registry['experiments'] "
                    "if item['id'] == 'TP-EXP-0022'); "
                    "assert development_review['state'] == 'COMPLETED'; "
                    "operator_handoff = next(item for item in registry['experiments'] "
                    "if item['id'] == 'TP-EXP-0023'); "
                    "assert operator_handoff['state'] == 'COMPLETED'; "
                    "preflight = next(item for item in registry['experiments'] "
                    "if item['id'] == 'TP-EXP-0024'); "
                    "assert preflight['state'] == 'COMPLETED'; "
                    "signing_session = next(item for item in registry['experiments'] "
                    "if item['id'] == 'TP-EXP-0025'); "
                    "assert signing_session['state'] == 'COMPLETED'; "
                    "from truepanel.holodeck.aegis_signing_session import "
                    "run_signing_session_checkride; "
                    "signing_proof = run_signing_session_checkride(); "
                    "assert signing_proof['status_counts']['HOLD'] == 9; "
                    "assert signing_proof['measurements']['false_ready'] == 0; "
                    "signing_tool = next(item for item in registry['experiments'] "
                    "if item['id'] == 'TP-EXP-0026'); "
                    "assert signing_tool['state'] == 'COMPLETED'; "
                    "from truepanel.holodeck.aegis_signing_tool import "
                    "run_signing_tool_checkride; "
                    "tool_proof = run_signing_tool_checkride(); "
                    "assert tool_proof['status_counts']['HOLD'] == 8; "
                    "assert tool_proof['measurements']['unsafe_ready'] == 0; "
                    "kit_audit = next(item for item in registry['experiments'] "
                    "if item['id'] == 'TP-EXP-0027'); "
                    "assert kit_audit['state'] == 'COMPLETED'; "
                    "from truepanel.holodeck.aegis_signing_kit_audit import "
                    "run_signing_kit_audit_checkride; "
                    "kit_proof = run_signing_kit_audit_checkride(); "
                    "assert kit_proof['status_counts']['HOLD'] == 10; "
                    "assert kit_proof['measurements']['review_card_substitutions_accepted'] == 0; "
                    "independent_audit = next(item for item in registry['experiments'] "
                    "if item['id'] == 'TP-EXP-0028'); "
                    "assert independent_audit['state'] == 'COMPLETED'; "
                    "from truepanel.holodeck.aegis_independent_kit_audit import "
                    "run_independent_kit_audit_checkride; "
                    "independent_proof = run_independent_kit_audit_checkride(); "
                    "assert independent_proof['status_counts']['HOLD'] == 12; "
                    "assert independent_proof['measurements']['unsafe_ready'] == 0; "
                    ...
                    "assert preflight_proof['status_counts']['HOLD'] == 19; "
                    "assert preflight_proof['status_counts'].get('UNSAFE_READY', 0) == 0; "
