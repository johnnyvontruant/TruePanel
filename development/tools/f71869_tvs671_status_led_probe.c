...
#define GPIO3_DIRECTION 0xC0
#define GPIO3_OUTPUT    0xC1
...
static void restore(void)
{
    if (runtime_access && value_changed) {
        runtime_write(GPIO3_OUTPUT, original_value);
        value_changed = 0;

        fprintf(stderr,
                "\nRestored IC1 to original value 0x%02X\n",
                original_value);
    }
}
...
int main(int argc, char **argv)
{
    ...
    direction = runtime_read(GPIO3_DIRECTION);
    original_value = runtime_read(GPIO3_OUTPUT);
    mask = (unsigned char)(1u << bit);

    printf("Direction IC0: 0x%02X\n", direction);
    printf("Original IC1:  0x%02X\n", original_value);
    printf("Candidate:     IC1 bit %u, %s status LED\n",
           bit,
           argv[1]);
    ...
    printf("Temporary IC1: 0x%02X\n", changed_value);
    printf("Hold time:     %u seconds\n", hold_seconds);
    printf("\nWatch the front status indicator.\n");

    runtime_write(GPIO3_OUTPUT, changed_value);
    value_changed = 1;

    readback = runtime_read(GPIO3_OUTPUT);
    printf("Readback IC1:  0x%02X\n", readback);
