...
def observe_button_event(event: ButtonEvent):
    """
    Record semantic button events without affecting production navigation.

    The existing QnapLCD callback remains authoritative while this shadow
    service validates debounce, release, hold, and repeat behavior against
...
def build_button_shadow_service(active_lcd):
    """
    Build the non-authoritative production button observer.
    """

    return ButtonService(
        active_lcd.read_buttons,
    ...
    try:
        ...
        if button_service is not None:
            try:
                ...
                LOGGER.exception(
                    "Button shadow service shutdown failed"
                )
            finally:
                button_service = None
