...
def get_arc_stats(self):
    ...
    def get_zfs_activity(self):
        ...
        if "resilver in progress" in lower:
            activity["resilver_running"] = True

        if "scrub in progress" in lower:
            activity["scrub_running"] = True

        for line in out.splitlines():
            stripped = line.strip()
            stripped_l = stripped.lower()

            if "scan:" in stripped_l:
                activity["status_line"] = stripped[:80]

            if "resilver in progress" in stripped_l or "scrub in progress" in stripped_l:
                activity["status_line"] = stripped[:80]

            if "%" in stripped and ("done" in stripped_l or "resilvered" in stripped_l or "scanned" in stripped_l):
            ...
            if "to go" in stripped_l:
                activity["remaining"] = stripped[:80]

            if any(word in stripped_l for word in ["degraded", "faulted", "unavail", "offline", "errors:"]):
                if "no known data errors" not in stripped_l and "all pools are healthy" not in stripped_l:
